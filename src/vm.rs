...
pub fn parse_manifest_yaml(input: &str) -> Result<VmManifest> {
    ...
    if !(1..=MAX_SUPPORTED_VM_MANIFEST_SCHEMA_VERSION).contains(&version) {
        bail!(
            "unsupported VM schemaVersion {version}; expected 1 to {MAX_SUPPORTED_VM_MANIFEST_SCHEMA_VERSION}"
        );
    }
    if version == 1
        && mapping
...
fn launch_interface_spec(interface: &VmInterface) -> VmLaunchInterfaceSpec<'_> {
    VmLaunchInterfaceSpec {
        ...
        source_mode: interface
            ...
            .as_ref()
            .copied()
            .map(VmInterfaceDirectMode::as_xml),
        ...
        if let Some(index) = matches.into_iter().next() {
            ...
            replacements.push(XmlReplacement {
                ...
                value: vm_reconcile::merge_interface_xml(
                    ...
                    node,
                    &desired_xml,
                    vm_reconcile::InterfaceManagedFields {
                        mac: interface.mac.is_some(),
                        source_mode: !interface.mode.is_preserve(),
                        vlan: !interface.vlan.is_preserve(),
                        mtu: !interface.mtu.is_preserve(),
                        link: !interface.link.is_preserve(),
                    },
                ),
            });
        } else {
    ...
    devices
        ...
        .filter_map(|interface| {
            let interface_type = match interface.attribute("type") {
                ...
                Some("direct") => VmInterfaceType::Direct,
                _ => return None,
            };
        ...
        .map(|(index, (interface, interface_type))| {
            let source_node = required_child(interface, "source")?;
            let source = source_node
                ...
                .to_string();
            let mode = source_node
                .attribute("mode")
                .map(|mode| match mode {
                    "vepa" => Ok(VmInterfaceDirectMode::Vepa),
                    "bridge" => Ok(VmInterfaceDirectMode::Bridge),
                    "private" => Ok(VmInterfaceDirectMode::Private),
                    "passthrough" => Ok(VmInterfaceDirectMode::Passthrough),
                    _ => bail!("unsupported domain XML direct interface mode {mode:?}"),
                })
                .transpose()?
...
fn current_domain_xml(connect_uri: &str, name: &str) -> Result<String> {
    ...
    ensure_supported_domain(&domain, name)?;
...
fn validate_manifest(manifest: &VmManifest) -> Result<()> {
    ...
    {
        bail!("machine.type microvm is no longer supported");
    }
...
fn validate_interfaces(interfaces: &[VmInterfaceEntry]) -> Result<()> {
    ...
    for entry in interfaces {
        ...
        if !interface.mode.is_preserve() && interface.interface_type != VmInterfaceType::Direct {
            bail!("interface {} mode requires type: direct", interface.id);
        }
        if let Some(vlan) = interface.vlan.as_ref() {
            if !(1..=4094).contains(vlan) {
                bail!("interface {} VLAN must be between 1 and 4094", interface.id);
            }
            let supported = interface.interface_type == VmInterfaceType::Bridge
                || (interface.interface_type == VmInterfaceType::Direct
                    && interface.mode.as_ref() == Some(&VmInterfaceDirectMode::Passthrough));
            if !supported {
                bail!(
                    "interface {} VLAN requires type: bridge or direct mode: passthrough",
                    interface.id
                );
            }
        }
...
fn list(args: VmListArgs) -> Result<()> {
    ...
    let mut rows = conn
        ...
        .into_iter()
        .filter_map(|domain| match domain_uses_unsupported_microvm(&domain) {
            Ok(false) => Some(Ok(domain)),
            Ok(true) => None,
...
fn lookup_domain(conn: &Connect, name: &str) -> Result<Domain> {
    let domain = Domain::lookup_by_name(conn, name)
        .with_context(|| format!("failed to find domain {name}"))?;
    ensure_supported_domain(&domain, name)?;
    Ok(domain)
}
...
fn lookup_domain_api(conn: &Connect, name: &str) -> VmApiResult<Domain> {
    match Domain::lookup_by_name(conn, name) {
        Ok(domain) => {
            if domain_uses_unsupported_microvm(&domain).map_err(VmApiError::Internal)? {
                Err(VmApiError::Conflict(anyhow::anyhow!(
                    "domain {name} uses unsupported machine.type microvm; manage it directly with libvirt"
                )))
            } else {
                Ok(domain)
            }
...
fn ensure_supported_domain(domain: &Domain, name: &str) -> Result<()> {
    if domain_uses_unsupported_microvm(domain)? {
        bail!(
            "domain {name} uses unsupported machine.type microvm; manage it directly with libvirt"
        );
    }
...
fn domain_uses_unsupported_microvm(domain: &Domain) -> Result<bool> {
    let xml = domain
        .get_xml_desc(sys::VIR_DOMAIN_XML_INACTIVE)
        .or_else(|_| domain.get_xml_desc(0))
        .context("failed to read domain XML while checking machine type")?;
    domain_xml_uses_unsupported_microvm(&xml)
}

fn domain_xml_uses_unsupported_microvm(xml: &str) -> Result<bool> {
    let document = Document::parse(xml).context("failed to parse domain XML")?;
    Ok(machine_from_domain_xml(document.root_element())
        .is_some_and(|machine| machine.machine_type == "microvm"))
}
...
"#;
    ...
    fn reconciles_multiple_interfaces_and_preserves_opaque_xml() {
        ...
        assert!(patched.contains("<interface type='direct'>"));
        assert!(patched.contains("<source dev='eno1' mode='bridge'/>"));
        ...
        manifest.interfaces = Some(vec![
            ...
            VmInterfaceEntry::absent("uplink"),
        ]);
        ...
        assert!(!removed.contains("ua-qtr-nic-uplink"));
...
"#;
        ...
        assert_eq!(interfaces[3].absent_id(), Some("retired"));
        ...
        let uplink = interfaces[2].as_present().unwrap();
        assert_eq!(uplink.interface_type, VmInterfaceType::Direct);
        assert_eq!(
            uplink.mode,
            VmOptionalValue::configured(VmInterfaceDirectMode::Passthrough)
        );
...
"#
    ...
    fn validate_manifest_checks_interface_identity_and_mac() {
        ...
        let interface = manifest.interfaces.as_mut().unwrap()[0]
            .as_present_mut()
            .unwrap();
        interface.interface_type = VmInterfaceType::Direct;
    ...
    #[test]
    fn rejects_unsupported_microvm_manifests() {
        let dir = TestDiskDir::new();
        let disk = dir.create_disk("sys.qcow2");
        let mut manifest = test_manifest(vec![test_file_disk(
            disk.to_str().unwrap(),
        ...
        assert!(error.to_string().contains("microvm is no longer supported"));
    }

    #[test]
    fn detects_unsupported_microvm_domain_xml() {
        let microvm = test_domain_xml().replace(
            "<type arch='x86_64'>",
            "<type arch='x86_64' machine='microvm'>",
        );
        assert!(domain_xml_uses_unsupported_microvm(&microvm).unwrap());
        assert!(!domain_xml_uses_unsupported_microvm(test_domain_xml()).unwrap());
    }
...
- id: primary
  type: user
"#;
        let error = parse_manifest_yaml(user_interface).unwrap_err();
        assert!(error.to_string().contains("failed to parse VM manifest"));
    }
